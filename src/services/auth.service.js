...
export const registerUser = async ({ name, email, password, phone }) => {
  ...
  // 1. Verificar unicidad del email ANTES de hashear
  ...
  // 2. Hashear la contraseña - Salt + Hash embebido
  const hashedPassword = await bcrypt.hash(password, SALT_ROUNDS);

  // 3. Enviar object prisma para crear el usuario
...
export const loginUser = async ({ email, password }) => {
  ...
  // 1. Buscar usuario -  Comparar contraseña
